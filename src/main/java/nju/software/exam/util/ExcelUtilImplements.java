...
public class ExcelUtilImplements implements ExcelUtil {
    ...
    public  List<question> readQuestionExcel(String filePath)
    {
        ...
        try
        {
          ...
          //  System.out.println("列数===>"+rs+"行数："+rows);
               Cell titleCell=st.getCell(0,0);
            String title=titleCell.getContents();
            System.out.println(title);
List<question> questionList=new ArrayList<question>();


            for(int k=2;k<rows&&!st.getCell(0,k).getContents().equals("");k++){//行
                question question=new question();
                question.setDescription(st.getCell(0,k).getContents());
                question.setOptionA(st.getCell(1,k).getContents());
                question.setOptionB(st.getCell(2,k).getContents());
                question.setOptionC(st.getCell(3,k).getContents());
                question.setOptionD(st.getCell(4,k).getContents());
                question.setAnswer(st.getCell(5,k).getContents());
                question.setLevel(st.getCell(6,k).getContents());
...
//            }
    ...
    public List<User> readStudentExcel(String filePath) {
        ...
        try
        {
            ...
            for(int k=2;k<rows&&!st.getCell(0,k).getContents().equals("");k++) {//行
                User user = new User();
                user.setEmail(st.getCell(0, k).getContents());
                user.setUsername(st.getCell(1, k).getContents());
                user.setGrade(Integer.parseInt(st.getCell(2, k).getContents()));
                user.setClaSS(Integer.parseInt(st.getCell(3, k).getContents()));
                userList.add(user);
...
//            //读Excel
//
//            ExcelUtilImplements.readExcel("/Users/fulinhua/Desktop/试题.xls");
//            //输出Excel
